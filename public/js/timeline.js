...
refresh_timeline = function() {
  return $.get('/api/all', function(data) {
    var completed, element, id, inverse, _i, _len;
    ...
    for (_i = 0, _len = data.length; _i < _len; _i++) {
      ...
      if (element.hasOwnProperty('completed')) {
        ...
        $('#timeline-elements').append(todo_element(id, inverse, "" + element.name + " - (" + completed + ")", '', element.time));
      } else if (element.hasOwnProperty('answer')) {
        $('#timeline-elements').append(qa_element(id, inverse, element.name, element.answer, element.time));
      } else {
        $('#timeline-elements').append(message_element(id, inverse, element.name, element.time));
      }
    }
  });
};

timeline_element = function(id, inverted, glyph, title, body, time) {
  var cls, tokens, _d, _t;
  cls = 'timeline-entry';
  if (inverted) {
...
message_element = function(id, inverted, title, time) {
  return timeline_element(id, inverted, 'glyphicon-pencil', title, '', time);
};

todo_element = function(id, inverted, title, body, time) {
  return timeline_element(id, inverted, 'glyphicon-list', title, body, time);
};

qa_element = function(id, inverted, title, body, time) {
